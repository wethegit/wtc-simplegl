...
import { createTransformFeedback } from "./createTransformFeedback.js";
...
const createRenderer = (
  ...
  const program = createProgram(gl, vert, frag, attributes, transformFeedbacks);
  const vao = createVertexArray(gl, program, attributes);
  const tfb = createTransformFeedback(gl, program, transformFeedbacks, Object.keys(attributes).length);
  let tfbI = 0;
  ...
  return {
    ...
    bindTransformFeedbacks() {
      if (!tfb) return;
      const i = (tfbI + 1) % 2;
      const source = tfb.vaos[tfbI];
      const feedbk = tfb.tfbs[i];
      const buffer = tfb.tfbBuffers[i];
    ...
    render({ width = null, height = null, x = 0, y = 0 } = {}) {
      ...
      if (tfb) gl.beginTransformFeedback(mode);
      gl.drawArrays(mode, offset, count);
      if (tfb) gl.endTransformFeedback();
    },
  };
};
